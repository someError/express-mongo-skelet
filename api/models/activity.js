...
exports.all = (req, callback) => {
 ...
 db.get().collection('activity').aggregate(pipeline).skip(skip).limit(_limit).toArray((err, docs) => {
  callback(err, docs)
 });
};

exports.findById = (id, callback) => {
 db.get().collection('activity').find({id}).sort({_id: -1}).toArray((err, docs) => {
  callback(err, docs)
 });
};
...
exports.findByQuery = (req, callback) => {
 ...
 db.get().collection('activity').aggregate(pipeline).toArray((err, docs) => {
  callback(err, docs)
 })
};

exports.create = (user, callback) => {
 db.get().collection('activity').insert(user, (err, res) => {
  callback(err, res)
 })
};

exports.update = (_id, data, callback) => {
 db.get().collection('activity').update({_id}, data, (err, res) => {
  callback(err, res)
 })
};

// exports.update = (id, data, callback) => {
//  db.get().collection('activity').findAndModify({
//   query: { id },
//   sort: { _id: -1 },
//   update: data,
//  }, (err, res) => {
